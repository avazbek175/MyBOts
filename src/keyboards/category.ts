...
export function categorySelectionKeyboard(categories: ICategory[], prefix: string = 'category:') {
...
export function categoryContentKeyboard(categorySlug: string, page: number, totalPages: number) {
  ...
  if (page > 1) {
    navButtons.push(Markup.button.callback(`${EMOJIS.prev} Oldingi`, `category_page:${categorySlug}:${page - 1}`))
  }

  navButtons.push(Markup.button.callback(`${page}/${totalPages}`, 'page_info'))

  if (page < totalPages) {
    navButtons.push(Markup.button.callback(`${EMOJIS.next} Keyingi`, `category_page:${categorySlug}:${page + 1}`))
  }
